 ...
 * @author Manuel Weber (ueberarbeitet)
 */
public class Receive {
     ...
     *
     * @param message Datenpaket, welches verarbeitet werden soll. Form: </br>
     * -OPCODE- -DATA-
     */
        ...
        if (message.length > 0) {
            ...
            switch (opcode) {
                case 0:
                    ...
                    // else if (message[1] == 1) {
                    // // Am besten abwarten bis Server abgearbeitet hat und erst am
                    // Ende
    ...
    private static void read0x04(byte[] message) {
        ...
        if (CommunicationUtils.bitIsSet(message[1], 7)) {
            ...
            // Fahrstorm aus
            /*	if ((VTfx.getMessung() != null)
					&& (VTfx.getMessung().isAlive() && (VTfx.getMessung()
							.isVerbindungsfehler() == false))) {
				vtfx.Funktionen.messungAbbrechen();
				JOptionPane
						.showMessageDialog(
								null,
								"Die Messung wurde abgebrochen, da in der RMX-PC-Zentrale der Fahrstrom abgeschaltet wurde.",
    ...
    private static void read0x08(byte[] message) {
        ...
        switch (opmode) {
            case 2: // Hex: 0x02
                // SX1: kurze Adresse, Steuerung ueber Loknummer
                // Fahrstufen: 31
                TrainDepotMap.getTrainDepot().addTrain(adrLong, adrShort, opmode,
                        (byte) 0, trainName, (byte) 0, (byte) 0, (byte) 0,
                        (byte) 0, 31);
                break;
            case 3: // Hex: 0x03
                ...
                break;
            case 5: // Hex: 0x05
                // SX1: kurze Adresse, Steuerung ueber Loknummer mit
            ...
            case 7: // Hex: 0x07
                ...
                break;
            case 9: // Hex: 0x09
                // DCC: 14 Fahrstufen, kurze Adresse, Steuerung ueber Loknummer
                TrainDepotMap.getTrainDepot().addTrain(adrLong, adrShort, opmode,
                        (byte) 0, trainName, (byte) 0, (byte) 0, (byte) 0,
                        (byte) 0, 14);
                break;
            case 10: // Hex: 0x0a
                ...
                break;
            case 12: // Hex: 0x0c
                // DCC: 28 Fahrstufen, kurze Adresse, Steuerung ueber Loknummer
                TrainDepotMap.getTrainDepot().addTrain(adrLong, adrShort, opmode,
                        (byte) 0, trainName, (byte) 0, (byte) 0, (byte) 0,
                        (byte) 0, 28);
                break;
            case 13: // Hex: 0x0d
                ...
                break;
            case 15: // Hex: 0x0f
                // DCC: 126 Fahrstufen, kurze Adresse, Steuerung ueber Loknummer
                TrainDepotMap.getTrainDepot().addTrain(adrLong, adrShort, opmode,
                        (byte) 0, trainName, (byte) 0, (byte) 0, (byte) 0,
                        (byte) 0, 126);
                break;
            ...
            case 31: // Hex: 0x1f
                /*
			 * Loeschen einer Lok ueber Loknummer: Angabe der alphanumerische
			 * Zeichen nicht erlaubt. <COUNT> = 7. Wird beim Loeschen die
			 * Loknummer 0000 eingegeben, wird die gesamte Datenbank geloescht.
                 */
                TrainDepotMap.getTrainDepot().removeTrain(adrLong);
                break;
            case 32: // Hex: 0x20
                /*
			 * Lesen eines Datensatzes ueber Loknummer: Angabe der
