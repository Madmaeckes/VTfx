...
public class TrainDepotMap {
	...
	protected synchronized void addTrain(int trainNumber, int adrShort,
			byte opmode, byte rmxChannel, String trainName, byte modeF0F7,
			byte modeF8F15, byte modeF16F23, byte direction, int runningNotch) {
	...
	protected synchronized ArrayList<String> generateTrainNameList() {
		...
		for (int i = 0; i < sortedList.size(); i++) {
			TrainNameList.add(TrainDepotMap.get(sortedList.get(i))
					.getTrainName());
		}
	...
	/**
	 * Gibt die Lok mit einem bestimmten Namen zurueck.
	 * 
	 * @param lokname
	 * @return lok
	 * @throws TrainNotFoundException
	 *             wenn die Lok nicht in der TrainDepotMap enthalten ist
	 */
	protected TrainObject getLok(String lokname) throws TrainNotFoundException {
		if (lokname == null) {
			throw new TrainNotFoundException();
		}
		for (TrainObject lok : TrainDepotMap.values()) {
			if (lokname.equals(lok.getTrainName())) {
				return lok;
			}
		}
		throw new TrainNotFoundException(lokname);
	}
}
